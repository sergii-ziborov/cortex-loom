...
- a bundled methodology library of nine workflows — test-driven development, systematic debugging, grounded review, evidence-first change, blast-radius analysis, interface-contract change, dependency upgrade, performance investigation, and incident response — seeded on first run, so the editor opens with working workflows rather than empty;
...
- a three-arm context benchmark (`cortex-bench`) measuring tokens and required-fact recall for no-tooling, Weavatrix-only, and Cortex Loom evidence assembly — see [benchmark](docs/benchmark.md) and [competitors](docs/competitors.md).
