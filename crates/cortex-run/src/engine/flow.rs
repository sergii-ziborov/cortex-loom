...
pub(crate) fn ensure_acyclic_flow(graph: &GraphDocument) -> Result<(), RunError> {
...
pub(crate) fn incoming_flow(graph: &GraphDocument) -> HashMap<&str, Vec<&str>> {
...
pub(crate) const fn is_flow_edge(kind: EdgeKind) -> bool {
    !matches!(
        kind,
...
pub(crate) const fn edge_matches(kind: EdgeKind, outcome: NodeOutcome) -> bool {
    match outcome {
