...
use operations::{
    ...
    neighbors_op, select_tests_op, share, stacktrace_op, symbol_op, verify_op,
};
...
fn plan_all(
    ...
    if !identifiers.is_empty() {
        operations.extend(operations::identifier_search_ops(
            task,
            symbol,
            &identifiers,
            search_budget,
    ...
    operations.extend(operations::task_specific_ops(task, &identifiers, policy));
    let symbol = symbol.filter(|name| crate::fold::is_graph_symbol(name));
