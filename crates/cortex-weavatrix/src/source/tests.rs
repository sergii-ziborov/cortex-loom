...
#[test]
fn an_ask_catalog_does_not_outrank_the_named_source_file() {
    let mut catalog = hit("crates/sweeploom-cli/src/agent_cases.rs", 57);
    catalog.text =
        r#"ask: "Split crates/sweeploom-cli/src/api.rs into two Rust modules.""#.to_owned();
    let mut api = hit("crates/sweeploom-cli/src/api.rs", 1);
    api.text = "pub fn serve_http(addr: SocketAddr)".to_owned();
    let unique = unique_paths_for_patterns(
        &[catalog, api],
        2,
    ...
    assert!(
        unique.iter().all(|hit| !hit.path.contains("agent_cases")),
        "catalog must not enter the packet: {unique:?}"
    );
    assert_eq!(unique[0].path, "crates/sweeploom-cli/src/api.rs");
}

#[test]
fn an_ask_catalog_is_dropped_when_it_is_the_only_hit() {
    let mut catalog = hit("crates/sweeploom-cli/src/agent_cases.rs", 57);
    catalog.text = r#"ask: "Find one real bug or dead path in the CLI crate.""#.to_owned();
    let unique = unique_paths_for_patterns(
        &[catalog],
        6,
    ...
    assert!(
        unique.is_empty(),
        "catalog-only hits must not open: {unique:?}"
    );
}

#[test]
fn a_task_that_names_the_catalog_may_open_it() {
    let catalog = hit("crates/sweeploom-cli/src/agent_cases.rs", 89);
    let unique = unique_paths_for_patterns(
        &[catalog],
        1,
        &[],
        "Fix is_silent_ask in crates/sweeploom-cli/src/agent_cases.rs",
    );
    assert_eq!(unique[0].path, "crates/sweeploom-cli/src/agent_cases.rs");
}

#[test]
fn catalog_matches_are_stripped_from_search_json() {
    let mut value = json!({
        "matches": [
            {
                "path": "crates/sweeploom-cli/src/agent_cases.rs",
                "line": 57,
                "text": "ask: Split api.rs"
            },
            {
    ...
    retain_product_search_matches(
        &mut value,
        "Split crates/sweeploom-cli/src/api.rs into modules",
    );
    let hits = hits_from_search(&value);
    assert_eq!(hits.len(), 1);
    assert_eq!(hits[0].path, "crates/sweeploom-cli/src/api.rs");
}
