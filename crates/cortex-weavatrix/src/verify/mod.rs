...
fn assess(
    ...
    if crate::plan_intent::asks_for_dead_production(task) {
        required_kinds.push(EvidenceKind::DeadCode);
    }
    if crate::plan_intent::asks_for_duplicate_share(task) {
        required_kinds.push(EvidenceKind::Duplicates);
    }
    ...
    missing.sort();
    missing.dedup();
    let definition_complete = symbol
        .filter(|name| crate::fold::is_graph_symbol(name))
        .is_some_and(|name| definition_group_complete(profile, name));
    ...
    if missing.iter().any(|item| item == "search_hits") {
        patterns.extend(
            extract_identifiers(task)
                .into_iter()
                .map(|identifier| crate::plan::search_code_query(&[identifier])),
        );
...
fn record_followup_coverage(
    ...
    if let Some(symbol) = symbol.filter(|name| crate::fold::is_graph_symbol(name)) {
        ...
        required.push(name.clone());
        if definition_group_complete(profile, symbol) {
            present.push(name);
        } else {
            missing.push(name);
        }
    ...
    if crate::plan_intent::asks_for_duplicate_share(task) {
        let name = "classifier_pair".to_owned();
        required.push(name.clone());
        if classifier_sites(profile).len() >= 2 {
            present.push(name);
        } else {
            missing.push(name);
        }
...
fn classifier_sites(profile: &EvidenceProfile) -> HashSet<String> {
    let mut names = HashSet::new();
    let haystack = format!("{}{}", profile.coverage_text, profile.identifier_text);
    for token in
        haystack.split(|character: char| !character.is_ascii_alphanumeric() && character != '_')
    {
        if token.contains("classify_") || token.starts_with("naive_") {
...
pub(crate) const fn kind_name(kind: EvidenceKind) -> &'static str {
    match kind {
        ...
        EvidenceKind::DeadCode => "dead_code",
        EvidenceKind::Duplicates => "duplicates",
        EvidenceKind::CoverageMap => "coverage_map",
    }
}
