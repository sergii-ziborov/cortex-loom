...
- Criticality attaches to the **head** sub-citation of a split fragment, not to every part of it. A symbol's definition can never be dropped by a budget; the tail of its reference list is high-priority and truncatable. Marking every part critical made a 4 000-token compile refuse outright whenever symbol evidence was present — measured, see [benchmark.md](benchmark.md).
- Which Weavatrix operations are asked is planned from the task text by `cortex_weavatrix::plan`, deterministically and without a model: identifiers are recognised by shape, lightweight intent cues select blast-radius (`get_dependents`) or API-contract (`list_endpoints`) tools when the question is structural, and the plan is a pure function of task, symbol, and budget. Each operation receives a share of the budget as a Weavatrix `token_budget`, so trimming happens where the tool knows which array to cut. A task naming no code plans structural evidence only, and says so rather than guessing.
- Weavatrix change-plan evidence remains unverified until an upstream agent or later verification phase resolves it. A truncated tail is reported in `omittedIds` and counted in `omittedEstimatedTokens`, so an omission is recorded rather than hidden.
