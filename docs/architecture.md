...
- `cortex-weavatrix`: native `weavatrix-rust` 2.17.4 evidence, typed conversion into the transport-independent context compiler, and bounded Rust validation/rendering of upstream-authored `weavatrix.refactor-plan.v1` plans. Every native call pins `expected_repository` to the session root. A task that names a file and a Git revision (`HEAD~N`, `HEAD^`, or a hex object id) also plans `git_read_blob`; definition lookup prefers `inspect_symbol` before a regex search. A blast-radius question also plans `find_references`, because `get_dependents` can return related neighbours that never name the function. A task that asks for measured line coverage plans `coverage_map` (ingest only: a missing report is unmeasured, not 0%). Weavatrix Quality (`wvq-*` 0.1.0-alpha.5) can write `.weavatrix/coverage/lcov.info` for that ingest; it is a sibling PROVE product, not a Cortex crate, and Cortex does not run tests. `ci_restrictions` / `explain_restriction` stay unplanned: they describe local GitHub Actions applicability, not a code change. n8n, Dify, Web3, Agent, and diagram packs stay unplanned. Source is grouped as `plan/`, `verify/`, `source/`, `compile/`, `core/`, `memory/`, and `adapter/` (with `adapter/support/`). Public crate paths are unchanged.
...
- A compiled packet has three layers: L0 decision map (`WX-MAP`: intent, targets, snapshot, required/satisfied/missing facets), L1 verified answer-bearing atoms, and L2 `EXPAND` handles for missing facets. Raw `graph_stats` dumps stay Low and do not occupy L1. The map is rendered from the selected set after L0/L2 take budget: compile, certify, layer, certify again until the ledger matches, so `WX-MAP` cannot claim a citation the packet then dropped.
- Completeness is a `CoverageCertificate`, not a boolean: required facets, the citation ids that close each one, missing facets, contradictions, and the snapshot. `sufficient` is derived; a critical missing facet still escalates. A kind is not a proof: empty dependents, an unrelated neighbour list, or `commits: 0` do not close `direct_callers` / `git_history`. Each close carries a `FacetClaim` (`validator`, `target`, `cardinality`).
- The agent `packetId` is the compiler's revision-bound `context.packetId`. `cortex_expand` refuses a stored handle when the tree snapshot moved (`packetStale`); it does not compile a new revision under the old identity.
