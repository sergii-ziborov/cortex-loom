...
- `cortex-weavatrix`: native `weavatrix-rust` 2.16.3 evidence, typed conversion into the transport-independent context compiler, and bounded Rust validation/rendering of upstream-authored `weavatrix.refactor-plan.v1` plans. Every native call pins `expected_repository` to the session root. A task that names a file and a Git revision (`HEAD~N`, `HEAD^`, or a hex object id) also plans `git_read_blob`; definition lookup prefers `inspect_symbol` before a regex search. A task that asks for measured line coverage plans `coverage_map` (ingest only: a missing report is unmeasured, not 0%). Weavatrix Quality (`wvq-*` 0.1.0-alpha.5) can write `.weavatrix/coverage/lcov.info` for that ingest; it is a sibling PROVE product, not a Cortex crate, and Cortex does not run tests. n8n, Dify, Web3, Agent, and diagram packs stay unplanned. Source is grouped as `plan/`, `verify/`, `source/`, `compile/`, `core/`, `memory/`, and `adapter/` (with `adapter/support/`). Public crate paths are unchanged.
