...
This rule is now enforced by the ledger itself: callers pass `runId` to `route_work` and `weavatrix_context_compile`, and the quality summary (`usage_read`, `GET /api/usage/quality`) joins attributed savings with run outcomes. A succeeded walk with no retries and no rejected gates is a `cleanRun` — that is not a quality proof. `qualityEquivalent` credit requires a clean run **and** an oracle (`hidden_tests`, `ci`, `review`, or `acceptance`) that passed with an artifact hash. Everything else stays `unprovenOmittedTokens`. First clean-run measurement: 4004 tokens on the dogfood run under a 4k budget with split fragments (budget utilization 1461 → 3545 selected tokens).
