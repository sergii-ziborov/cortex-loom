...
5. ~~Import a small, licensed Superpowers-derived methodology fixture set and build scenario/pressure tests for round-trip behavior.~~ Done: three original methodology skills in the Superpowers `SKILL.md` format (`crates/cortex-skills/fixtures/` with a provenance NOTICE; no upstream prose copied) plus pressure tests: semantic round-trip with an export fixpoint check, CRLF and Unicode stability, a 60-step dependency chain, and hostile-frontmatter rejection. The tests immediately caught a real sharp edge — `[depends: N]` is a tail annotation, punctuation after it breaks label stability — now documented by the fixtures themselves.
6. Stabilize or publish native Rust Refactor planning crates before removing the JavaScript compatibility oracle. Probed 2026-08-04 against `weavatrix-rust` 2.1.1 (now pinned): still no native refactor planning operations (`rename_symbol`/`apply_edit_plan`/etc. are absent from the crate source), so the preview-only JavaScript oracle stays; this item remains blocked on upstream Weavatrix work. The 2.1.x line does add `build_graph`, `map_stacktrace`, `select_tests`, and build-system awareness (justfile/makefile) — `map_stacktrace` and `select_tests` are strong future evidence sources for failure triage and test selection.
