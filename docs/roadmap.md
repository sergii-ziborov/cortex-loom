...
4. Add embeddings and retrieval evaluation before permitting semantic evidence selection. Iterations 1–2 done: bounded `/api/embed`, repository-specific Recall@k/nDCG fixtures (32 corpus docs, 12 queries, structural relatedness pairs), and three pinned ranking modes — pure embedding, hybrid (RRF of embedding + BM25), and hybrid with a rank-space graph boost that can never overtake its benefactor. Measured: pure embedding fails the gate on both models (recall@5 0.875); `hybrid_graph` **passes** on both — `embeddinggemma` recall@5 0.96 / nDCG@5 0.92 and `qwen3-embedding:0.6b` recall@5 1.00 / nDCG@5 0.96 (hybrid alone also passes on qwen). Semantic selection is unblocked for the passing mode; wiring it into the evidence compiler is iteration 3 and stays deterministic-first with the omission report intact.
