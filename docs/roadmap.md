...
4. Add embeddings and retrieval evaluation before permitting semantic evidence selection. Iteration 1 done: bounded `/api/embed` support in cortex-ollama, repository-specific Recall@k/nDCG fixtures (32 corpus docs, 12 queries), pure retrieval metrics, and a gated verdict (`recall@5 ≥ 0.9`, `nDCG@5 ≥ 0.75`) in cortex-eval. First measurement: `embeddinggemma` reaches MRR 1.00 and nDCG@5 0.87 but recall@5 0.875 — FAIL by the recall gate, so semantic selection stays blocked until a profile passes.
